    ...
    if not is_train:
        ...
        for n in dataset_names:
            ...
            for s in dataset_types:
                tmp = []
                # if islocal:
                sizefilepath_ = f"{dataset_path}/{s}/{n}/sizes.json"
                ...
                for k in sizes.keys():
                    # if islocal:
                    tmp.append(f"{dataset_path}/{s}/{n}/{k}")
                    # else:
                        # tmp.append(
        ...
        tmp = []
        if islocal:
            sizefilepath_ = f"{dataset_path}/sizes.json"
            if not os.path.exists(sizefilepath_):
                sizefilepath_ = f"./json_files/sizes.json"
        else:
            sizefilepath_ = f"./json_files /sizes.json"
        # if not os.path.exists(sizefilepath_):
        #     continue
        ...
        for k in sizes.keys():
            if islocal:
                tmp.append(f"{dataset_path}/{k}")
            else:
                # tmp.append(
                #     f"pipe:aws s3 --cli-connect-timeout 0 cp s3://s-laion-audio/webdataset_tar/{k} -"
                # )
                a = 1
        if proportion != 1:
            tmp = random.sample(tmp, int(proportion * len(tmp)))
