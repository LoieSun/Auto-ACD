...
import sys
sys.path.append('/mnt/petrelfs/sunluoyi/autoacd/code/CLAP_distribute/src/laion_clap/')
...
def main():
    ...
    print(args.horovod, args.distributed)
    if args.horovod:
        logging.info(
...
def lp_main(args, device, writer, pretrain_epoch, idx):
    ...
    args.enable_fusion = True
    args.fusion_type = 'aff_2d'
    ...
    # model = clap_model().to(device_id)
    # model = DDP(model, device_ids=[device_id])
    model = clap_model.to(device)
    ...
    if "train" not in data:
        ...
        metric = evaluate(model, data, 0, args, writer)
        if is_master(args):
            best_metrics = update_metric(best_metrics, metric)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if is_master(args):
            print(f"after training epoch {epoch}")
