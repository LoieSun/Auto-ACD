...
# text random mask on words
def random_mask_sentence(sentence, mask_prob):
    tokens = sentence.split() 

    mask = torch.rand(len(tokens)) < mask_prob
    masked_tokens = [token if not masked else "[MASK]" for token, masked in zip(tokens, mask)]
    masked_sentence = " ".join(masked_tokens)  

    return masked_sentence

def random_mask_sentences(sentences, mask_prob):
    
    masked_sentences = [random_mask_sentence(sentence, mask_prob) for sentence in sentences]
...
def preprocess_single(
    ...
    if is_train:
        ...
        if random_number == 0:
            ...
            transforms = [
                ...
                HighLowPass(sample_rate=orig_sr), # this augmentation will always be applied in this aumgentation chain!
                RandomApply([Delay(sample_rate=orig_sr)], p=0.3),
                RandomApply([PitchShift(
                    n_samples=num_samples,
                    sample_rate=orig_sr
                )], p=0.3),
                RandomApply([Reverb(sample_rate=orig_sr)], p=0.3)
            ]
    ...
    # print('vgg-' in sample['__url__'])
    json_dict_raw = sample[text_ext]

    texts = select_text(json_dict_raw, text_augment_selection)
    if is_train:
        # if 'vgg-' in sample['__url__'] or 'audioset-' in sample['__url__']:
        # print(texts)
        mask_prob = 0.25
        if type(texts) == list:
            texts = random_mask_sentences(texts, mask_prob)
        else: 
            texts = random_mask_sentence(texts, mask_prob)
        # print(texts)
...
def collate_fn_with_preprocess(batch,
    ...
    for sample in batch:
        # print(sample)
        audio_data, orig_sr = sample[audio_ext]
        # audio_data = sample[audio_ext]
        if len(audio_data) == 0:
            print('aaa:',audio_data)
            ...
            data_preprocessed.append(
                preprocess_single(sample, audio_ext, text_ext, max_len, audio_cfg, tmodel, class_index_dict, data_filling,
                                  data_truncating, text_augment_selection, is_train))
...
def get_data(args, model_cfg):
    ...
    # args.val_dataset_names = 'vggsound'
    args.val_dataset_names = ['Clotho', 'audiocaps']
    dataset_path = '/data/train/tar'
    val_dataset_path = '/autoacd/data/'
