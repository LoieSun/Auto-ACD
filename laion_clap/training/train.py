...
def train_one_epoch(
    ...
    for i, batch in enumerate(dataloader):
        ...
        if isinstance(optimizer, dict):
            ...
            optimizer.zero_grad()
        # if is_master(args):
            # logging.info("after optimizer zero grad")
        with autocast():
            (
                audio_features,
                text_features,
            ...
            # logging.info(f"rank {args.rank}, after model forward")
            ...
            # logging.info(f"{args.rank}, after loss calculation")
        if isinstance(optimizer, dict):
            # if is_master(args):
                # logging.info("first branch")
            if scaler is not None:
                # if is_master(args):
                    # logging.info("scaler not None")
        ...
        # logging.info(f"rank {args.rank}, after backward")
        ...
        # valid in one epoch
        # args.valid_in_one_epoch_frequency = 200
        # if ( i % args.valid_in_one_epoch_frequency) == 0:
        #     evaluate_in_one_epoch(model, data, epoch, i, args, tb_writer=None)
        # print('done')

    # end for


def evaluate_in_one_epoch(model, data, epoch, batch_index, args, tb_writer=None):
    metrics = {}
    print(f"parallel eval: {args.parallel_eval}")
    ...
    with torch.no_grad():
        ...
        if is_master(args):
            ...
            for n in eval_info.keys():
                ...
                val_metrics_in_one_epoch[n] = {
                    n + "/" + k: v for k, v in metrics_single_dataset.items()
                }
                metrics.update(val_metrics_in_one_epoch[n])
    ...
    if is_master(args):
        ...
        logging.info(
            f"Eval Batch: {epoch}-{batch_index}  "
            + "\n".join(
                [
                    "\t".join([f"{k}: {round(v, 4):.4f}" for k, v in m.items()])
                    for m in val_metrics_in_one_epoch.values()
                ]
            )
        )
        ...
        if args.wandb:
            assert wandb is not None, "Please install wandb."
            for name, val in metrics.items():
                wandb.log({f"val/{name}": val, "batch_index": f'{epoch}-{batch_index}'})
...
def evaluate(model, data, epoch, args, tb_writer=None):
    ...
    if args.val_dataset_names == ['Clotho', 'audiocaps']:
        ...
        print('evaluate after every epoch')
        dataloader = data["val"].dataloader
        num_samples = 0
        samples_per_val = dataloader.num_samples
...
def calculate_selection_performance_clotho_audiocaps(val_metrics_per_dataset, args):
    """
    Calculate performance for Clotho+AudioCaps for model selection.
    """
    selection_performance_all = []
    print('top_k_checkpoint_select_metric', args.top_k_checkpoint_select_metric)
    for n in val_metrics_per_dataset.keys():
        selection_performance = (val_metrics_per_dataset[n][f"{n}/audio_to_text{args.top_k_checkpoint_select_metric}"] +
                                 val_metrics_per_dataset[n][f"{n}/text_to_audio{args.top_k_checkpoint_select_metric}"]) / 2
...
def select_top_metric_clotho_audiocaps(metrics, val_metrics_per_dataset, args):
    ...
    if not hasattr(args, "top_selection_performance"):
        selection_performance = calculate_selection_performance_clotho_audiocaps(val_metrics_per_dataset, args)
        ...
        selection_performance_new = calculate_selection_performance_clotho_audiocaps(val_metrics_per_dataset, args)
